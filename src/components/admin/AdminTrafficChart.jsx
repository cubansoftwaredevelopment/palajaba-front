...
export default function AdminTrafficChart({
  ...
  return (
    <section className={adminCard}>
      <div className="mb-4">
        <h3 className="text-sm font-medium text-zinc-50">Tráfico del marketplace</h3>
        <p className={`mt-1.5 text-xs leading-relaxed ${adminSubtle}`}>
          Visitantes únicos por sesión en /comprar (home). Un mismo visitante cuenta una vez al día.
        </p>
      </div>
        ...
        <>
          <div className="mb-3">
            ...
            <p className="mt-1 text-2xl font-semibold tabular-nums text-zinc-50 sm:text-3xl">
              {(chart.total_visits ?? 0).toLocaleString('es')}
            </p>
          ...
          <div className="h-56 w-full min-w-0 sm:h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={points} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="admin-traffic-fill" x1="0" y1="0" x2="0" y2="1">
